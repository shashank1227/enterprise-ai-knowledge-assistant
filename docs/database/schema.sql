-- ============================================================
-- Enterprise AI Knowledge Assistant - Database Schema
...
CREATE TABLE users (
    ...
    oauth_subject   VARCHAR(255),                        -- provider user id
...
CREATE TABLE roles (
    id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name        VARCHAR(50) NOT NULL UNIQUE,             -- ADMIN, USER, VIEWER
    description VARCHAR(255),
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

INSERT INTO roles (name, description) VALUES
    ('ADMIN',  'Full system access including user management and admin dashboard'),
    ('USER',   'Can upload documents, search, and chat'),
    ('VIEWER', 'Read-only access to search and chat');
...
CREATE TABLE knowledge_sources (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name            VARCHAR(255) NOT NULL,               -- "Confluence", "SharePoint", "Upload"
    source_type     VARCHAR(50) NOT NULL,                -- UPLOAD, CONFLUENCE, SHAREPOINT, S3
...
CREATE TABLE documents (
    ...
    tags                TEXT[],
    metadata            JSONB,                           -- custom metadata key/value
...
CREATE TABLE document_chunks (
    ...
    content_tokens      INT,
    page_number         INT,
    section_title       VARCHAR(500),                    -- extracted heading
...
-- IVFFlat as backup for exact search (uncomment for smaller datasets)
...
CREATE TABLE messages (
    ...
    parent_message_id   UUID REFERENCES messages(id),   -- for regeneration tracking
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE message_citations (
    ...
    citation_index  INT NOT NULL,                        -- [1], [2], [3] in response
    relevance_score FLOAT NOT NULL,
    excerpt         TEXT,                                -- the specific text cited
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE message_feedback (
    ...
    comment         TEXT,
    feedback_type   VARCHAR(50),                         -- HELPFUL, INACCURATE, INCOMPLETE, HARMFUL
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX idx_analytics_query_fts ON query_analytics
...
CREATE TABLE audit_logs (
    ...
    resource    VARCHAR(100),                            -- DOCUMENT, USER, CONVERSATION
    resource_id UUID,
    details     JSONB,
    ip_address  INET,
...
INSERT INTO system_config (key, value, description) VALUES
    ...
    ('chat_model',            'gpt-4.1', 'OpenAI chat completion model'),
    ('max_context_tokens',    '8000', 'Maximum tokens sent to LLM as context'),
...
-- Hybrid search function: combines vector similarity + full-text BM25-style
...
BEGIN
    ...
    rrf_scores AS (
        SELECT
            ...
            COALESCE(1.0 / (rrf_k + t.rank), 0)     AS hybrid_score
